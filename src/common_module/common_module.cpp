...
#pragma omp parallel for
  for (row = 0; row < size; ++row)
  {
    ...
    if (k - 1 >= 0)
    {
      ...
      csrRowOffsets[row + 1]++;
      csrColInd[row * STENCIL_WIDTH + csrRowOffsets[row + 1]] =
          col;
      csrValues[row * STENCIL_WIDTH + csrRowOffsets[row + 1]] =
          static_cast<T>(-mean_k);
    }
    if (k + 1 < P)
    {
      col = i * N * P + j * P + k + 1;
    ...
    if (k == 0 || k == P - 1)
      csrValues[row * STENCIL_WIDTH] +=
          static_cast<T>(2.0 * k_z[row]);
    csrRowOffsets[row + 1]++;
  }
  ...
  for (row = 0; row < size; ++row)
  {
    std::memmove(&csrColInd[csrRowOffsets[row]],
                 &csrColInd[row * STENCIL_WIDTH],
                 sizeof(int) * csrRowOffsets[row + 1]);
    std::memmove(&csrValues[csrRowOffsets[row]],
                 &csrValues[row * STENCIL_WIDTH],
                 sizeof(T) * csrRowOffsets[row + 1]);
    csrRowOffsets[row + 1] += csrRowOffsets[row];
  }
}

template int getCsrMatData<float>(
    std::vector<int> &csrRowOffsets,
    std::vector<int> &csrColInd,
...
template int getCsrMatData<double>(
    std::vector<int> &csrRowOffsets,
    std::vector<int> &csrColInd,
...
template int getStdRhsVec<float>(
    std::vector<float> &rhs,
    const std::vector<int> &dims,
    const std::vector<double> &k_z,
    const double delta_p);

template int getStdRhsVec<double>(
...
#pragma omp parallel for reduction(+ : homoCoeffZ)
  for (i = 0; i < M; ++i)
    ...
    {
      row = i * N * P + j * P;
      temp = 2.0 * lenZ * lenZ / (M * N * P) * k_z[row] *
             (static_cast<double>(p[row]) - delta_p) / delta_p;
...
template int getHomoCoeffZ<float>(
    float &homoCoeffZ,
...
template int getHomoCoeffZ<double>(
    double &homoCoeffZ,
